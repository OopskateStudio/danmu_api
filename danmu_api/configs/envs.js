...
如果没有找到合适的匹配，返回：
{
  ...
  static resolveSourceOrder() {
    let sourceOrder = this.get('SOURCE_ORDER', '360,tencent,iqiyi,youku,bilibili,dandan', 'string');
    ...
    return orderArr.length > 0 ? orderArr : ['360', 'tencent', 'iqiyi', 'youku', 'bilibili', 'dandan'];
  }
  ...
  static load(env = {}) {
    ...
    const envVarConfig = {
      ...
      // 源配置
      'SOURCE_ORDER': { category: 'source', type: 'multi-select', options: this.ALLOWED_SOURCES, description: '源排序配置，默认360,tencent,iqiyi,youku,bilibili,dandan' },
